...
int main(void)
{
  ...
  void UART_Printf(UART_HandleTypeDef *huart, const char* fmt, ...) {
      ...
      va_start(args, fmt); // Memulai mengambil argumen variabel
      vsnprintf(buff, sizeof(buff), fmt, args); // Format string ke buffer
      HAL_UART_Transmit(huart, (uint8_t*)buff, strlen(buff), HAL_MAX_DELAY); // Transmit menggunakan UART yang dipilih
      va_end(args); // Mengakhiri mengambil argumen variabel
  }
  void UART_SendAll(){
    UART_Printf(&huart1,"UART1 >>> EGT = %u CHAMBER = %u\n\r", MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo1), MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo2));
    UART_Printf(&huart2,"UART2 >>> EGT = %u CHAMBER = %u\n\r", MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo1), MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo2));
    UART_Printf(&huart3,"UART3 >>> EGT = %u CHAMBER = %u\n\r", MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo1), MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo2));
    UART_Printf(&huart4,"UART4 >>> EGT = %u CHAMBER = %u\n\r", MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo1), MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo2));
    UART_Printf(&huart5,"UART5 >>> EGT = %u CHAMBER = %u\n\r", MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo1), MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo2));
    UART_Printf(&huart6,"UART6 >>> EGT = %u CHAMBER = %u\n\r", MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo1), MAX6675_ConvertRawDataToTempIntDeg(&max6675_termo2));
